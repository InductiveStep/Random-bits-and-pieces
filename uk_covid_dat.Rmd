...
```{r}
last_seg <- last_seg %>%
  mutate(start_date = min(as.Date(date)),
         day = as.numeric(as.Date(date) - start_date))

last_seg %>%
  ggplot(aes(day, newCasesBySpecimenDate)) +
  geom_point() +
  geom_smooth()
```

```{r}
grouped <- peaks_valleys_dat %>%
  group_by(date_segment) %>%
  mutate(start_date = min(as.Date(date)),
         day = as.numeric(as.Date(date) - start_date)) %>%
  ungroup()
...
```{r}
focal_segments <- c("2021-04-25","2020-07-21")
```


```{r}
segs_admissions <- grouped %>%
  filter(date_segment %in% focal_segments) %>%
  ggplot(aes(day, newAdmissions, colour = date_segment)) +
  geom_smooth() +
  geom_point() +
  labs(x = "Days since start of period",
       y = "Hospital admissions",
       colour = "Start of period")
```
...
grouped %>%
  ...
  group_by(date_segment) %>%
  filter(day == days_in) %>%
  pull(date) %>% as.Date() %>% format("%d %b %Y")
```
...
segs_cases <- grouped %>%
  filter(date_segment %in% focal_segments) %>%
  ggplot(aes(day, newCasesBySpecimenDate, colour = date_segment)) +
  geom_smooth(se = FALSE) +
  geom_point() +
  labs(x = "Days since start of period", y = "Cases", colour = NULL)
```
...
segs_deaths <- grouped %>%
  filter(date_segment %in% focal_segments) %>%
  ggplot(aes(day, newDeaths28DaysByDeathDate,
             colour = date_segment)) +
  geom_smooth(se = FALSE) +
  geom_point() +
  labs(x = "Days since start of period", y = "Deaths", colour = NULL)
```


```{r fig.height=6, fig.width=6, dpi=300}
segs_admissions / segs_cases
```

```{r fig.height=6, fig.width=6, dpi=300}
segs_admissions / segs_deaths / segs_cases
```
...
```{r}
covid_hosp %>%
  ggplot(aes(as.Date(date), covidOccupiedMVBeds)) +
  geom_point(size = .5)
```
...
```{r}
covid_hosp %>%
  ggplot(aes(as.Date(date), hospitalCases)) +
  geom_point(size = 0.5)
```
...
```{r}
recent_dat <- covid_hosp %>%
  filter(as.Date(date) >= as.Date("2021-01-01"))

min(recent_dat$hospitalCases)
```

```{r}
min_date <- recent_dat %>%
  filter(hospitalCases == min(recent_dat$hospitalCases)) %>%
  pull(date)
min_date
```

```{r}
current_wave <- covid_hosp %>%
  filter(as.Date(date) >= as.Date(min_date)) %>%
  mutate(days_in = as.Date(date) - as.Date(min_date),
         days_in = as.integer(days_in))
```



```{r}
current_wave$days_in
```


```{r}
current_wave %>%
  ggplot(aes(days_in, log(hospitalCases,2))) +
  geom_point()
```
...
```{r}
lm1 <- lm(log2(hospitalCases) ~ days_in + I(days_in^2),
          data = current_wave)
summary(lm1)
```
...
```{r}
predicted_hospital_cases <- data.frame(
  days_in = 0:365
) %>%
  mutate(date = as.Date(min_date) + days_in)

predicted_hospital_cases$predicted_cases <- 2^predict(
  object = lm1,
  newdat = predicted_hospital_cases)
```

```{r}
predicted_hospital_cases %>%
  filter(predicted_cases <= 41000)
```
...
```{r}
current_wave %>%
  ggplot(aes(days_in, hospitalCases)) +
  geom_point() +
  geom_function(fun = \(x) 2^(coef(lm1)[1] +
                                coef(lm1)[2]*x +
                                coef(lm1)[3]*x^2)) +
  xlim(0, 82)
```
