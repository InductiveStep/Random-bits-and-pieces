...
Now aggregate the weights to make it easier to check for balance. This yields the same weights as those produced by psmatch2 and kmatch.
...
```{r}
merge(bal_unadj$Balance |> select(-Diff.Adj,-Type),
      bal_adj$Balance   |> select(-Diff.Un,-Type),
      by=0, all=TRUE) |>
  rename(Matched = Diff.Adj,
         All     = Diff.Un) |>
  pivot_longer(cols = -Row.names) |>
  ggplot(aes(x = abs(value), y = factor(Row.names), color = name)) +
  geom_point() +
  labs(y = "",
       x = "|SMD|",
       color = "")
...
The outcome model, as [tentatively suggested](https://cran.r-project.org/web/packages/MatchIt/vignettes/estimating-effects.html) by MatchIt authors for another approach using matching with replacement.
...
```{r}
avg_comparisons(lm(re78 ~ treat, data = outdat, weights = .wt),
                variables = "treat",
                vcov = ~ .id,
                newdata = subset(outdat, treat == 1),
                wts = ".wt")
