...
# Beginning with something easy on MatchIt

Follow through with an example -- this is the non-matching:
...
summary(m0)
```
5:1 nearest neighbor matching with replacement:
...
The outcome model, as advised by MatchIt author(s) for another approach using matching with replacement. Note interaction between treat and covariates. This is a doubly-robust approach. Leave out the covariates if you don't want that.
...
Don't trust this -- the sample size is wildly inflated and the SEs don't yet take this into account:
