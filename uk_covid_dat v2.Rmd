...
peaks_valleys_dat_temp <- peaks_valleys_dat %>%
  select(i, newCasesBySpecimenDate) %>%
  na.omit()

peaks_valleys_dat_temp$newCasesBySpecimenDate_smooth =
  sma(peaks_valleys_dat_temp$newCasesBySpecimenDate,
      order = sma_order)$fitted

the_peaks <- peaks_valleys_dat_temp$i[
  findPeaks(peaks_valleys_dat_temp$newCasesBySpecimenDate_smooth,thresh = 2)]
the_valleys <- peaks_valleys_dat_temp$i[
  findValleys(peaks_valleys_dat_temp$newCasesBySpecimenDate_smooth, thresh = 2)]

switches <- c(the_peaks,
              the_valleys) %>%
  sort()
...
switches <- c(1, switches[select_em],max(peaks_valleys_dat$i))
plot(peaks_valleys_dat$newCasesBySpecimenDate)
abline(v = switches)
```
