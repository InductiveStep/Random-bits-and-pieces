...
hexstr2chars <- function(hexstr) {
  bytevec <- substring(hexstr,
                       seq(1, nchar(hexstr) - 1, 2),
                       seq(2, nchar(hexstr), 2))
  strtoi(bytevec, base = 16) |>
    as.raw() |>
    rawToChar()
}
```
...
These two functions translate back and forth between 0..f (hex) and a..p (lethex):

```{r}
hex2lethex <- function(hstr) {
  indat <- data.frame(hex = (hstr |> strsplit(""))[[1]])
  outdat <- indat |>
    left_join(charmap, by = "hex")
  outdat$let |> paste(collapse = "")
}

lethex2hex <- function(lethex) {
  indat <- data.frame(let = (lethex |> strsplit(""))[[1]])
  outdat <- indat |>
    left_join(charmap, by = "let")
  outdat$hex |> paste(collapse = "")
}
```
