...
That works for the estimates(!), but not quite the SEs (still surprised they are that close). There's only one ATT(g, t) for group 2007, so I don't get why the pooled SE is different to that one observation. But that seems to be a glue. Anyway, time to revisit the paper and peek at the [package code](https://github.com/bcallaway11/did).
